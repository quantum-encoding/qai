...
Options:
  ...
  -o <file>         Output file (default: stdout)`)
	...
	if hasKt && !hasGo && !hasTS && !hasPy && !hasSwift {
		...
		ktResults, err := AnalyzeKtTree(absDir)
		if err != nil {
	...
	if hasSwift && !hasGo && !hasTS && !hasPy {
		...
		swiftResults, err := AnalyzeSwiftTree(absDir)
		if err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
	...
	if hasPy && !hasGo && !hasTS {
		...
		pyResults, err := AnalyzePyTree(absDir)
		if err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
	...
	if hasTS && !hasGo {
		...
		tsResults, err := AnalyzeTSTree(absDir)
		if err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
	...
	results, err := AnalyzeGoTree(absDir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		os.Exit(1)
	}
...
func formatComplexity(results []*GoAnalysis) string {
	type entry struct {
		Name       string
		File       string
		Complexity int
...
func formatCallGraph(results []*GoAnalysis) string {
	var b strings.Builder
...
func formatTypes(results []*GoAnalysis) string {
	var b strings.Builder
	for _, r := range results {
		rel := filepath.Base(r.File)
...
func formatAnalysisMD(results []*GoAnalysis, root string) string {
	var b strings.Builder
...
func formatSummary(results []*GoAnalysis, root string) string {
	var b strings.Builder
	name := filepath.Base(root)
...
func detectLanguages(dir string) (hasGo, hasTS, hasPy bool) {
	filepath.WalkDir(dir, func(path string, d os.DirEntry, err error) error {
		if err != nil || d.IsDir() {
			if d != nil && d.IsDir() && ignoreDirs[d.Name()] {
...
func formatPyOutput(results []*PyAnalysis, root, format string, showComplexity, showCalls, showTypes bool) string {
	switch {
	case showTypes:
		var b strings.Builder
		for _, r := range results {
...
func formatTSOutput(results []*TSAnalysis, root, format string, showComplexity, showCalls, showTypes bool) string {
	switch {
	case showTypes:
...
func formatTSSummary(results []*TSAnalysis, root string) string {
...
func formatTSTypes(results []*TSAnalysis) string {
	var b strings.Builder
	for _, r := range results {
		rel := filepath.Base(r.File)
		for _, iface := range r.Interfaces {
...
func formatTSCalls(results []*TSAnalysis) string {
	var b strings.Builder
	b.WriteString("Call Graph:\n\n")
	for _, r := range results {
		for _, edge := range r.CallGraph {
...
func formatTSComplexity(results []*TSAnalysis) string {
	type entry struct {
		Name       string
		File       string
		Complexity int
...
func formatTSMarkdown(results []*TSAnalysis, root string) string {
	var b strings.Builder
	name := filepath.Base(root)
...
func formatTSParamList(params []TSParam) string {
	var parts []string
...
func formatSwiftSummary(results []*SwiftAnalysis, root, format string) string {
	totalFns, totalClasses, totalEdges := 0, 0, 0
	for _, r := range results {
		totalFns += len(r.Functions)
		totalClasses += len(r.Classes)
