...
func embedAllChunks(chunks []chunk) [][]float64 {
	...
	for i := 0; i < len(chunks); i += embedBatchSize {
		...
		embs, err := embedTextsDispatch(texts)
		if err != nil {
...
// All embedding calls now go through embedTextsDispatch().

// ─── SurrealDB storage ──────────────────────────────────────────────────────

func storeSurreal(conn surrealConn, provider string, chunks []chunk, embeddings [][]float64) {
	fmt.Printf("Clearing existing %q in SurrealDB...\n", provider)
	surrealExec(conn, fmt.Sprintf("DELETE FROM doc_chunk WHERE provider = '%s';", escapeSurQL(provider)))
...
func surrealExec(conn surrealConn, sql string) {
...
func searchLocalSurreal(query, provider string, limit int) {
	...
	// Build vector search SQL — filter by dimension to avoid mismatched comparisons.
	...
	if provider != "" {
		whereClauses = append(whereClauses, fmt.Sprintf("provider = '%s'", escapeSurQL(provider)))
	}
	sql := fmt.Sprintf(
	...
	resp, err := (&http.Client{Timeout: 120 * time.Second}).Do(req)
	if err != nil {
		fmt.Fprintf(os.Stderr, "qai search --local: SurrealDB connection failed: %v\n", err)
	...
	if len(results) == 0 {
		...
		// Hint: show what dimensions are available.
		dimResult := dbQuery("SELECT dimension, count() AS n FROM doc_chunk GROUP BY dimension;")
		if dimResult != "" {
...
func ingestPrecomputed(conn surrealConn, provider, archiveDir string) {
	// Locate files.
...
func ensurePrecomputedSchema(conn surrealConn) {
	surrealExec(conn, `
