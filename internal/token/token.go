...
func RefreshToken(creds *gcpADC) (*gcpTokenResp, error) {
	params := url.Values{
	...
	if resp.StatusCode != 200 {
		...
		return nil, fmt.Errorf("token refresh %d: %s (run: gcloud auth application-default login)", resp.StatusCode, strutil.TruncateStr(string(body), 200))
	}
...
func refreshViaSA() (*gcpTokenResp, error) {
	out, err := platform.ReadCredentialFromKeystore()
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("SA token exchange %d: %s", resp.StatusCode, strutil.TruncateStr(string(body), 200))
	}
...
func gcpIdentityToken(creds *gcpADC, audience string) (*gcpTokenResp, error) {
	...
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("identity token %d: %s", resp.StatusCode, strutil.TruncateStr(string(body), 200))
	}
