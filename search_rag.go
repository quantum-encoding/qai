...
func ragSearch(args []string) {
	...
	for _, c := range searchCorpora {
		...
		if resp.StatusCode != 200 {
			fmt.Fprintf(os.Stderr, "  search %s: %d %s\n", c.DisplayName, resp.StatusCode, truncateStr(string(respBody), 200))
			continue
		}
...
func ragListCorpora(base, token string) ([]ragCorpus, error) {
	...
	for {
		...
		if resp.StatusCode != 200 {
			return nil, fmt.Errorf("list corpora %d: %s", resp.StatusCode, truncateStr(string(body), 200))
		}
